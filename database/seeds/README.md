...
**Pipeline automatizado que usa Gemini para generar datos realistas e insertarlos en PostgreSQL o MongoDB.**
...
![Kubectl](https://img.shields.io/badge/kubectl-required-326CE5?style=for-the-badge&logo=kubernetes&logoColor=white)
...
Este directorio contiene el script que **genera datos de prueba realistas** usando la **API de Gemini (Google)** e **inserta los datos** en PostgreSQL o MongoDB.

> El script corre en **tu máquina local** y se comunica con los pods del cluster de Kubernetes a través de `kubectl exec`.
>
> **No necesita `psql` ni `mongosh` instalados localmente** — esos binarios ya existen dentro de los pods (`postgres:16-alpine` incluye `psql`; `mongo:7` incluye `mongosh`).
...
- El stack desplegado con [`deploy.ps1`](../../infra/k8s/README.md) **antes** de correr el seed
...
*Si desplegaste en uno distinto al default:*
...
> Para **MongoDB** se conecta al pod **`mongos`** *(el query router del sharded cluster)*, no directamente a un shard. Esto es importante porque mongos es quien conoce la distribución de datos y rutea cada documento al shard correcto según la **shard key** (`idMenu` hashed para `menuitems`, `idRestaurante` hashed para `reservations`).
...
> Si `kubectl exec` falla por algún motivo, podés ejecutar los pasos manualmente **una vez que los archivos intermedios ya fueron generados**.
